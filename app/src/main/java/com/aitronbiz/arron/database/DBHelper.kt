...
class DBHelper(context: Context?) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
   ...
   override fun onCreate(db: SQLiteDatabase) {
      val user = "create table $USER(id integer primary key autoincrement, type text, idToken text, accessToken text, " +
         "username text, email text, role text, contact text, emergencyContact text, createdAt text);"
      db.execSQL(user)

      val device = "create table $DEVICE(id integer primary key autoincrement, uid integer, subjectId integer, name text, " +
         "productNumber text, serialNumber text, activityTime integer, status text, sign integer, createdAt text, updatedAt text);"
      db.execSQL(device)
